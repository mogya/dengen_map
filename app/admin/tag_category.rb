ActiveAdmin.register Tag::Category do
  permit_params :name, :importance, :image, :sprite_pos, :type, :detail, :parent_id
  config.sort_order = 'parent_id_asc'
  config.per_page = 80
  index do
    render partial: 'active_admin/tag/categories/tree'
  end

  filter :image
  filter :detail
  filter :parent, as: :check_boxes, collection: proc { Tag::Category.order(:parent_id, :importance) }
  filter :updated_at
